...
func AcquireConsumer(db *sql.DB, cfg *config.Config, resumptionTime int64, useBlockTime bool, pl *plugins.PluginLoader) (streamsTransports.Consumer, error) {
	...
	if cfg.LightSeed != 0 {
		...
		consumer, err := deliverConsumer(brokerParams, resumption, reorgThreshold, resumptionTime, int64(1), hash, lastWeight, append(trackedPrefixes, ptp...), cfg.WhitelistExternal)
		if err != nil {

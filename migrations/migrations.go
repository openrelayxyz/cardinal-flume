	...
	if schemaVersion < 7 {
		...
		case 137:
			if _, err := db.Exec(fmt.Sprintf(`INSERT INTO blocks.baseFeeDenominatorSchedule(startBlock, endBlock, denominator) VALUES (%v, %v, %v)`, 0, 38189055, 8)); err != nil {
				log.Error("migrations hoodi INSERT INTO blocks.baseFeeDenominatorSchedule v1 error", "err", err.Error())
				return nil
			}
		...
		default:
			if _, err := db.Exec(fmt.Sprintf(`INSERT INTO blocks.baseFeeDenominatorSchedule(startBlock, endBlock, denominator) VALUES (%v, %v, %v)`, 0, maxInt, 8)); err != nil {
				log.Error("migrations default INSERT INTO blocks.baseFeeDenominatorSchedule v1 error", "err", err.Error())
				return nil
			}
		}
		if _, err := db.Exec("UPDATE blocks.migrations SET version = 7;"); err != nil {

...
func (api *LogsAPI) GetLogs(ctx context.Context, crit FilterQuery) ([]*logType, error) {
	...
	if highestTopic == 0 && len(topicsClause) > 0 && len(addressClause) > 0{
		...
		if topicCount < addrCount {
			...
			log.Debug("AddrQuery", "q", addrQuery, "blockparams", blockParams, "addrParams", addressParams)
			log.Debug("TopicQuery", "q", topicQuery, "blockparams", blockParams, "topicParams", topic0Params)
		} else {
